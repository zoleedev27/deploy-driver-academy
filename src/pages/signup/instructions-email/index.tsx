...
import { VerificationToken } from "@/constants/MockUsersVerified";
...
const VerificationsEmail : NextPageWithLayout = () => {
    ...
    useEffect(() => {
        ...
        const tokens : VerificationToken[] = JSON.parse(localStorage.getItem("mockVerificationTokens") || "[]");
        ...
        <div className="flex items-center justify-center min-h-screen font-body text-black dark:text-white py-10">
            <Card className="w-full max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2x">
                <CardHeader className="space-y-1">
                    ...
                    <CardTitle className="text-2xl text-center">{t("sign.up.instructions.email.title")}</CardTitle>
                ...
                <div className="bg-muted p-4 rounded-lg">
                    <h3 className="mb-2">{t("sign.up.instructions.email.next.steps.title")}</h3>
                    ...
                    {canResend ? (
                        <Button variant="link" onClick={handleResendEmail} className="p-0 h-auto">
                            <div className = "flex items-center gap-1 justify-center hover:text-green-400">
                                ...
                                <span>{t("sign.up.instructions.email.resend.link.text")}</span>
                            </div>
                        </Button>
                    ):(
                    <p className="text-orange-500 dark:text-orange-300">{t("sign.up.instructions.email.resend.timer", {countdown})}</p>
                    )}
                </div>
                ...
                {mockToken && (
                  ...
                  <AlertDescription className="text-sm text-amber-800 dark:text-amber-100">
                    <p className="mb-1 font-medium">{t("sign.up.instructions.email.test.title")}</p>
                    <p className="mb-2">
