...
export const usePagination = ({totalItems, routePrefix, allowedPerPageOptions = [5, 10, 15, 20]}: UsePaginationProps) => {
  ...
  const itemsPerPage = allowedPerPageOptions.includes(parsedPerPage) ? parsedPerPage : allowedPerPageOptions[0];
  ...
  useEffect(() => {
    ...
    const isValidPage = !isNaN(parsedPage) && parsedPage >= 1 && parsedPage <= totalPages;
  ...
  }, [parsedPage, parsedPerPage, routePrefix, router, itemsPerPage, totalPages, currentPage]);
  ...
  const pagesToDisplay = useMemo(() => {
    ...
    const midRange = Array.from({ length: 3 }, (_, i) => currentPage - 1 + i).filter((p) => p > 1 && p < totalPages);
